...
from utils.validator import validate_dish_name, validate_servings

async def log_calorie(user_id: int, req: CalorieRequest, db: Session):
    validate_dish_name(req.food_name)
    validate_servings(req.servings)
    ...
    if key in cache:
        ...
        calories_per_serving = await fetch_calories(req.food_name)
